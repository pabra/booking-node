...
module.exports = function (args) {
    ...
    if (!args.tokenObservable) throw new Error('missing "tokenObservable" in args');
    if (!args.containerElement) throw new Error('missing "containerElement" in args');
    if (!args.modelName) throw new Error('missing "modelName" in args');

    args.containerElement.innerHTML = template.replace(/\{\{\s*modelName\s*\}\}/g, args.modelName);
    ...
    self.getItems = function () {
        xhr({
            ...
            data: JSON.stringify({token: args.tokenObservable()}),
            responseType: 'json',
            headers: {
